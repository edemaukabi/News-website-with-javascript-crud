...
const generateCommentUI = (articles) => {
  ...
  for (let item of sortedArticles) {
    ...
    commentRow.innerHTML = `<p class="comment-text ${item.id}">${
      item.comment
    }</p>
    <form class="comment-update-form ${item.id}">
        <textarea class="comment-text-update ${item.id}" name="commentUpdate">${
      item.comment
    }</textarea>
        <div class="error"></div>
    ...
    <div class="comment-details">
        <p class="comment-author ${item.id}">Commented by <span>${
      item.name
    }</span></p>
...
commentsWrap.addEventListener("click", async (e) => {
  ...
  if (e.target.classList.contains("comments-edit")) {
    commentUpdateForm = document.getElementsByClassName(
      `comment-update-form ${targetClass}`
    )[0];

    commentUpdateForm.style.display = "flex";
    commentText = document.getElementsByClassName(
      `comment-text ${targetClass}`
    )[0];
    commentText.style.display = "none";
    commentEditBtn = document.getElementsByClassName(
      `comments-edit ${targetClass}`
    )[0];
    commentEditBtn.style.display = "none";
    commentDeleteBtn = document.getElementsByClassName(
      `comments-delete ${targetClass}`
    )[0];
    commentDeleteBtn.style.display = "none";

    commentUpdateForm.addEventListener("submit", submitFunc);
    commentTextUpdated = document.getElementsByClassName(
      `comment-text-update ${targetClass}`
    )[0];
    commentOwner = document.getElementsByClassName(
      `comment-author ${targetClass}`
    )[0];
  }
  ...
  if (e.target.classList.contains("comments-delete")) {
    ...
    if (response.ok) {
      ...
      tempNodes = commentsWrap.querySelectorAll(`.comment-row`);
...
const deleteComment = async (id) => {
  let response = await fetch(
    `https://61924d4daeab5c0017105f1a.mockapi.io/credo/v1/news/${news_id}/comments/${id}`,
    {
      method: "DELETE",
    }
  );
...
const submitFunc = (e) => {
  ...
  if (errorContent.commentUpdate) {
    ...
    fetch(commentURL, {
      method: "PUT",
      body: JSON.stringify({
        name: commentOwner.firstChild.nextSibling.innerText,
        comment: commentTextUpdated.value,
        newsId: news_id,
      }),
      headers: {
